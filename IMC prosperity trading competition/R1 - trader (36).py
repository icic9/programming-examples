...
class Trader:
    """
    v31: v30 robustness baseline + synthetic-tested adverse-carry guard.
    ...
    Design principles:
      ...
      - PEPPER has an additional adverse-carry guard calibrated from synthetic
        flat/down trend stress tests. It is inactive on the observed up-carry
        regime, so current-data PnL remains identical to v30.
      - OSMIUM fill-quality learning uses both timestamp and tick horizons,
        making it less dependent on a specific timestamp step size.
    """
    ...
    def _trade_osmium(
        ...
        # State-policy guard: only when inventory is very stretched and the
        # book pressure is against adding more of the same inventory, stop
        # showing the wrong side. This keeps normal participation unchanged.
        if position >= 64 and toxicity < -0.40:
            remaining_buy = 0
        elif position <= -64 and toxicity > 0.40:
            remaining_sell = 0
