...
class Trader:
    """
    v33: v31 robustness framework + OSMIUM Bayesian stationary prior.

    Built on v31 (which itself layered robustness on v30). Inherits all of v31:
      - PEPPER carry remains the default. Flat/short regimes require multi-signal
        confirmation. Adverse-carry guard (inactive on current uptrend data) protects
        against regime reversals.
      - OSMIUM fill-quality learning uses both timestamp and tick horizons.

    OSMIUM innovations (new in v33, from v32 analysis):
      1. Bayesian stationary prior: OSMIUM is ARIMA(0,0,0) around 10000 (empirical
         mean=10000.20, std=9.22 across all days). Adding a static 10000 anchor with
         0.16 weight is the correct Bayesian correction for a zero-drift OU process.
         When wall_mid temporarily deviates from 10000, the prior pulls fair back
         toward the unconditional mean, reducing adverse quote placement.
         Weights: 0.46 wall_mid + 0.20 microprice + 0.18 learned_anchor + 0.16 static(10000).

      2. OSMIUM_BASE_WIDTH=0: removes the artificial 1-tick minimum edge when one side
         of the book is momentarily empty. Captures passive flow in thin markets while
         the penny-step logic continues to enforce appropriate edge when the full
         9992/10008 structure is present.
    """
