...
class Trader:
    """
    v9: robust, low-parameter, data-driven market maker

    Design principles:
      1) No hardcoded absolute price anchors.
      2) Inventory managed via reservation price, not brittle signals.
      3) OSMIUM fair uses an anti-lag EMA forecast:
           fair = ema - c * (mid - ema)
         This is level-free and exploits the strong EMA-gap reversion seen
         in the training data without pinning to 10,000 or any fixed level.
      4) Queue priority matters more than complex layering in wide-spread books,
         so OSMIUM quotes one level only.
      5) When inventory is stretched, quote-or-not-quote:
         suppress the wrong side, tighten the exit side.
    """
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, saved: dict) -> List[Order]:
        ...
        # Adaptive EMA center. Use microprice because it is the least overfit
        # level-free forecast signal in the released R1 books.
        a = self.OSMIUM_EMA_ALPHA
        if saved["osmium_ema"] is None:
            saved["osmium_ema"] = microprice
        else:
            saved["osmium_ema"] = a * microprice + (1.0 - a) * saved["osmium_ema"]
        ...
        logger.print(
            f"OSM| mid={mid:.1f} mp={microprice:.1f} ema={ema:.2f} fair={fair:.2f} "
    ...
    def _trade_pepper(self, od: OrderDepth, position: int, saved: dict) -> List[Order]:
        ...
        logger.print(
            ...
            f"eff={eff_ema:.2f} vol={vol:.2f} pb={passive_buy} ps={passive_sell} pos={position}"
        )
