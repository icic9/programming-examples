...
class Trader:
    """
    v7-R2: R1 v31 baseline + R2 mechanics + OSMIUM structural dislocation trading.
    ...
    R2 changes:
      ...
      - v6 separates maker and taker reservation: range/dislocation signals
        steer passive quotes, while crossing uses only fair value plus inventory.
      - v7 deliberately keeps that executable logic unchanged after the
        official 332881 sample improved OSMIUM.  PEPPER and OSMIUM follow
        separate macro/micro models; no single-run PnL noise is fed back into
        coefficients.
    ...
    def bid(self) -> int:
        ...
        Top 50% of all bids win: they pay the fee and receive 25% extra order
        book exposure (100% visible vs default 80%).  Bidding 200 is likely
        above the median while keeping net cost small relative to expected PnL.
        """
        return 200
    ...
    def _trade_osmium(
        ...
        inventory_reservation = fair - self.OSMIUM_INV_SKEW * (position / limit)
        base_reservation = inventory_reservation - self.OSMIUM_RANGE_SKEW * range_factor
        # Taker decisions use only fair/inventory.  Range and wall dislocation
        # are maker alpha; using them to cross the book pays spread for a signal
        # whose robust expression is passive fill selection.
        ...
        # Take only obvious fair/inventory gifts.  Dislocation/range signals are
        # applied below to passive quote placement and side sizing.
