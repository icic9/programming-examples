...
class Trader:
    """
    v26 robust adaptive redesign

    Key robustness upgrades vs v25:
      - PEPPER no longer assumes a permanent long carry regime.
      - It can build a long core, flat book, or short core depending on
        online signals (EMA trend + rolling OLS slope + trade flow).
      - No fixed positive clock-drift prior.
      - OSMIUM keeps the wall/regime-maker philosophy, but fixes the wall-mid
        estimate and uses real inventory regimes (neutral / loaded / stressed).
      - No hardcoded price levels anywhere.
    """
    ...
    def run(self, state: TradingState):
        ...
        saved.setdefault("pepper_regime", 0)   # -1 short core, 0 flat, +1 long core
    ...
    def _trade_pepper(
        ...
        regime = -1 if strong_reversal else 1
        ...
        logger.print(
            ...
            f"reg={regime} core={self.PEPPER_CORE_TARGET} "
    ...
    def _trade_osmium(
        ...
        logger.print(
            ...
            f"tox={toxicity:.2f} reg={regime} pb={passive_buy} ps={passive_sell} pos={position}"
        )
