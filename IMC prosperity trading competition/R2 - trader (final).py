"""
Final Round 2 trading strategy for IMC Prosperity 4.

Reader orientation:
- The competition engine calls Trader.run(state) once per timestamp.
- The strategy returns limit orders plus serialized state in traderData.
- INTARIAN_PEPPER_ROOT is traded as a drift/carry product.
- ASH_COATED_OSMIUM is traded as a range-reversion and microstructure product.
- The file is intentionally self-contained because submissions must be uploaded
  as a single Python file using the official datamodel interface.
"""
...
class Logger:
    """
    Compact logger for the Prosperity visualizer.

    The official log size is limited, so the logger compresses state and orders
    into short JSON arrays and truncates diagnostic text when necessary.
    """
...
class Trader:
    ...
    Finance summary:
      - PEPPER: carry a long core while clock drift and trend remain healthy;
        recycle a smaller sleeve with passive market-making.
      - OSMIUM: estimate a range-bound fair value and express small alpha
        mostly through passive quote price and size rather than by crossing.
    ...
    def _penny_passive_prices(od: OrderDepth, ref: float, fair: float,
        ...
        """
        Choose passive prices that improve the visible queue by one tick while
        preserving a minimum edge around fair value.

        This is the core market-making placement rule: join the market close
        enough to get filled, but do not cross into prices that erase expected
        edge.
        """
    ...
    def _update_osmium_fill_quality(
        ...
        """
        Track whether recent OSMIUM fills aged well.

        A buy fill is good if the later mark is above the fill price; a sell
        fill is good if the later mark is below the fill price. The resulting
        bias slightly adjusts fair value so the strategy becomes more cautious
        on the side that has recently been adversely selected.
        """
    ...
    def run(self, state: TradingState):
        """
        Main exchange callback.

        Decode persistent state, route each product to its strategy, encode the
        updated state, and return orders. Any rolling statistic that must live
        across timestamps is stored in `saved` and returned as traderData.
        """
        result = {}
    ...
    def _trade_pepper(
        ...
        """
        Trade PEPPER as a drift/carry asset.

        The strategy estimates a clock-driven fair value, keeps a core long
        allocation while the drift regime is healthy, and uses warning counters
        to flatten or turn defensive when trend/flow/clock signals deteriorate.
        """
        orders: List[Order] = []
    ...
    def _trade_osmium(
        ...
        """
        Trade OSMIUM as a range-bound microstructure product.

        Fair value comes from wall-mid, microprice, and a learned range anchor.
        Range, hot-quote, and dislocation signals mostly affect passive quote
        placement and side sizing; crossing the spread is reserved for clearer
        fair-value gaps.
        """
        orders: List[Order] = []
