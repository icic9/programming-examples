...
class Trader:
    """
    v2-R2 (v12): (10) baseline + (11)'s robustness upgrades, without the
    dip-buyer.

    Kept from (11):
      - PEPPER_ONE_SIDE_OBS_WEIGHT: downweights EMA/drift updates on
        one-sided books, avoiding skewed fair under thin quotes.
      - One-sided reference applied on every tick, not only at init.
      - Kinematic alpha-beta tracker (pepper_fast_*) used for fast-break.
      - Fast-break early flat/short trigger on sharp downside kinematics.

    Dropped from (11):
      - Dip-buyer (DIP_* constants, pepper_dip_count, target_core + 8 bump).
        Added local PnL in-sample but violates robustness-first: bigger entry
        at adverse points if the trend fails to recover.

    OSMIUM is unchanged from (10)/(11).
    """
    ...
    OSMIUM_HOT_TAKE_THRESH = 0.40
    OSMIUM_HOT_EDGE_REDUCTION = 1
    OSMIUM_NEAR_EMA_ALPHA = 0.15
    OSMIUM_HOT_L1L2_THRESH = 5
    OSMIUM_HOT_EMA_THRESH = 4
    ...
    def run(self, state: TradingState):
        ...
        saved.setdefault("osmium_near_bid_ema", None)
        saved.setdefault("osmium_near_ask_ema", None)
        saved.setdefault("osmium_hot_signal", 0.0)
    ...
    def _trade_pepper(
        ...
        logger.print(
            ...
            f"brk={saved['pepper_carry_break_count']} "
