...
class Trader:
    """
    v0-R2: R1 v31 baseline + R2 mechanics + granular OSMIUM mean-reversion.
    ...
    R2 changes:
      ...
      - OSMIUM hot-quote signal: tracks near-bid and near-ask EMAs; when L1
        bid jumps well above L2 bid AND above EMA (transient aggressive buyer),
        flags a mean-reversion opportunity on the sell side. Symmetrically for
        asks. Used to tighten sell/buy edge when the hot signal is strong.
    ...
    def _trade_osmium(
        ...
        # R2: tighten edge on the mean-reversion side when hot signal is strong
        ...
        # Only reduce when take threshold is already > 0 (don't give edge away)
        if hot > self.OSMIUM_HOT_TAKE_THRESH:
            sell_take = max(0, sell_take - self.OSMIUM_HOT_EDGE_REDUCTION)
            sell_edge = max(1, sell_edge - self.OSMIUM_HOT_EDGE_REDUCTION)
        elif hot < -self.OSMIUM_HOT_TAKE_THRESH:
            buy_take = max(0, buy_take - self.OSMIUM_HOT_EDGE_REDUCTION)
        ...
        # Recompute reservation after any takes
        reservation = (
            fair
            - self.OSMIUM_INV_SKEW * (position / limit)
            - self.OSMIUM_RANGE_SKEW * range_factor
        ...
        logger.print(
            ...
            f"rfac={range_factor:.2f} hot={hot:.2f} "
