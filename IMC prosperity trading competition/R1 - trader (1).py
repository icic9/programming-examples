...
class Trader:
    ...
    # ── PEPPER (INTARIAN_PEPPER_ROOT) ─────────────────────────────────────
    PEPPER_LIMIT          = 80
    PEPPER_EMA_ALPHA      = 0.50    # ARIMA(0,1,1)-optimal: ACF(1)=−0.501 → α=0.499
    PEPPER_EMA_SLOW_ALPHA = 0.06    # sustained trend EMA (unchanged from Tutorial v6)
    PEPPER_SKEW           = 0.025   # ticks per position unit; max ±2 at limit, ±4 in urgency
    ...
    PEPPER_URGENCY_MULT     = 2.0   # skew multiplier in urgency zone
    PEPPER_TREND_CARRY      = 0.65  # Round 1 sweep: stronger carry fixes stale-fair underbuying in trend
    PEPPER_TREND_GATE       = 1.0   # vs 1.5 in Tutorial; signal≈1.47 ticks → throttle fires early
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, saved: dict) -> List[Order]:
        ...
        if saved["osmium_ema"] is None:
            ...
            saved["osmium_ema"] = alpha * microprice + (1.0 - alpha) * saved["osmium_ema"]
    ...
    def _trade_pepper(self, od: OrderDepth, position: int, saved: dict) -> List[Order]:
        ...
        if saved["pepper_ema"] is None:
            saved["pepper_ema"] = microprice
        else:
            saved["pepper_ema"] = a_fast * microprice + (1.0 - a_fast) * saved["pepper_ema"]
        ema = saved["pepper_ema"]

        # Slow EMA — sustained trend tracker
        a_slow = self.PEPPER_EMA_SLOW_ALPHA
        if saved["pepper_ema_slow"] is None:
            saved["pepper_ema_slow"] = microprice
        else:
            saved["pepper_ema_slow"] = (
                a_slow * microprice + (1.0 - a_slow) * saved["pepper_ema_slow"]
            )
