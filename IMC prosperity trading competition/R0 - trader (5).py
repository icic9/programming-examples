"""
IMC Prosperity 4 — Strategy v5: Dual-EMA Momentum Signal
=========================================================
Products: EMERALDS (stationary ~10000) | TOMATOES (ARIMA(0,1,1) drift)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  CONTEXT: RESEARCH PROCESS AND DISCARDED CHANGES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  This version follows the v4 microstructure foundation and records the
  main candidates tested before the retained v5 changes.

  DISCARDED 1 — At-fair EMERALDS aggressive fills.
  ...
  DISCARDED 2 — Full-book microprice (total volume weights).
       ...
       occasional EMA misdirection. Reverted to level-1 microprice.

  DISCARDED 3 — 1-tick EMA velocity for momentum.
       ema_velocity = EMA_t − EMA_{t−1} is dominated by noise. Improved
       day -2 (+138 TOMATOES) but hurt day -1 (-131). Net: roughly neutral.
       Replaced with the dual-EMA signal below.

  RETAINED 1 — Dual-EMA momentum signal (Innovation 3).
  RETAINED 2 — Urgency position skew (Innovation 4).
  RETAINED 3 — Exponential moving variance (Innovation 5).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  FOUNDATIONS CARRIED FORWARD FROM V4 (unchanged)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  - EMERALDS: identical to v4 (no at-fair fills)
  ...
  - Vol-adaptive spread widening (threshold rarely triggered)
...
class Trader:
    """
    v5: Dual-EMA momentum signal for TOMATOES inventory management.

    Key structural change: adds ema_slow (α=0.10) alongside existing
    ema_fast (α=0.43). Their difference is the trend signal driving
    a momentum-biased effective position, which in turn adjusts the
    inventory skew for passive quote placement.
    ...
    TOMATO_EMA_SLOW_ALPHA = 0.10   # slow EMA; eff. memory ≈ 10 ticks
    ...
    def _trade_emeralds(self, od: OrderDepth, position: int) -> List[Order]:
        """
        EMERALDS v5: identical to v4. Known fair = 10,000 (DF t-stat ≈ -97, white noise).
    ...
    def _trade_tomatoes(
        ...
        """
        TOMATOES v5: dual-EMA momentum signal + urgency skew + EMV.

        Fair-value (unchanged from v4):
        ...
        Innovation 5 — Exponential moving variance:
          emv = 0.15 × diff² + 0.85 × emv_prev.
          Replaces the 20-element rolling list from v4.

        Innovation 3 — Dual-EMA trend signal:
          ema_slow: EMA(α=0.10) of the same microprice.
        ...
        Aggressive phase: raw ema_fast only (no momentum adjustment).
        Passive phase: eff_ema derived from skew(biased_pos).
        """
        orders: List[Order] = []
        ...
        # ── Phase 1: Aggressive — raw EMA only (unchanged from v4) ───────
        # Momentum signal is a passive inventory management tool only.
        # Aggressive fills remain disciplined at raw fair value.
        for ask in sorted(od.sell_orders):
            if ask >= ema: break
            can_buy = limit - position
            if can_buy <= 0: break
        ...
        # ── Phase 2: Passive — dual-EMA momentum-biased, urgency-scaled ───
        ...
        logger.print(
            ...
            f"slow={ema_slow:.2f} sig={trend_signal:.2f} bpos={biased_pos:.1f} "
