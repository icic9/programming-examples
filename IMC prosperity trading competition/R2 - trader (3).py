...
class Trader:
    """
    v3-R2: R1 v31 baseline + R2 mechanics + macro/micro separated alpha.
    ...
    R2 changes:
      ...
      - PEPPER fair is clock-dominant because the path is almost deterministic
        at +0.1/tick across the three R2 days; EMA/flow remain sleeve controls.
      - OSMIUM absolute-level reversion changes inventory appetite and crossing
        permission, not passive quote price. The quote stays at the matching
        engine's best capture level.
    ...
    # Absolute OSMIUM level has robust long-horizon mean reversion around 10000.
    # Use it as inventory/crossing appetite, not passive quote fair.
    OSMIUM_ABS_ANCHOR = 10000.0
    OSMIUM_ABS_BAND = 6.0
    ...
    def _trade_osmium(
        ...
        abs_dev = mid - self.OSMIUM_ABS_ANCHOR
        abs_side = 0
        if abs_dev >= self.OSMIUM_ABS_BAND:
            abs_side = -1
        elif abs_dev <= -self.OSMIUM_ABS_BAND:
            abs_side = 1
        ...
        if abs_side < 0 and position > -self.OSMIUM_ABS_CROSS_POS_ALLOW:
            can_take_ask = False
        if can_take_ask:
            for ask in sorted(od.sell_orders):
        ...
        logger.print(
            ...
            f"absD={abs_dev:.1f} absS={abs_side} "
