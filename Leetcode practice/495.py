...
class Solution:
    ...
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        # iterative solution
        res = duration
        poison_til = timeSeries[0] + duration - 1

        for i in range(1, len(timeSeries)):
            new_poison_til = timeSeries[i] + duration - 1
            
            if (poison_til < timeSeries[i]):
                res += new_poison_til - timeSeries[i] + 1
            else:
                res += new_poison_til - poison_til  # the "poison_til" day is already counted

            poison_til = new_poison_til
